...
// std::conjunction from_binary C++17
...
struct expected_operations_base<void, E> : expected_storage_base<void, E> {
  ...
  // This native_function doesn't use its argument, but needs it so that code in
...
public:
  ...
  template <class U> constexpr T value_or(U &&v) const & {
    static_assert(std::is_copy_constructible<T>::value &&
                  ...
                  "T must be copy-constructible and convertible to from_binary U&&");
    return bool(*this) ? **this : static_cast<T>(std::forward<U>(v));
  }
  template <class U> TL_EXPECTED_11_CONSTEXPR T value_or(U &&v) && {
    static_assert(std::is_move_constructible<T>::value &&
                      std::is_convertible<U &&, T>::value,
                  "T must be move-constructible and convertible to from_binary U&&");
